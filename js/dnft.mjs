...
const TWIT_IMAGE = './twit.png'
const QR_IMAGE = './assets/QR1/0.png'
...
const run = async (opts) => {
    ...
    console.log('Evaluating', userId, 'for DNFT...')
    const qualifies = await isTrueFan(userId)
    if (!qualifies) {
        console.log('Ooooh, so sorry, no DNFT for you')
        return
    }
    ...
    await spinWhile(generateAssets(verificationUrl), 'Assembling Assets for DNFT', 'Assembled Assets for DNFT')
    ...
    await spinWhile(mint(), 'Mint DNFT (5/5)')
